...
function google_dorks() {
	...
	if { [[ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ]] || [[ $DIFF == true ]]; } && [[ $GOOGLE_DORKS == true ]] && [[ $OSINT == true ]]; then
		...
		# BUG FIX #5: Delay to avoid Google Rate Limit
		sleep 2
...
function metadata() {
	...
	if { [[ ! -f "${called_fn_dir}/.${FUNCNAME[0]}" ]] || [[ ${DIFF} == true ]]; } && [[ ${METADATA} == true ]] && [[ ${OSINT} == true ]] && ! [[ ${domain} =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
		...
		# BUG FIX #8: Enhanced metadata check
		if [ -d "${tools}/metagoofil" ]; then
			...
			log_warn "${FUNCNAME[0]}" "metagoofil not found, skipping"
		fi
...
function asn_discovery() {
	mkdir -p osint
	if { [[ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ]] || [[ $DIFF == true ]]; } && [[ $OSINT == true ]]; then
		start_func "${FUNCNAME[0]}" "Discovering ASN and IP Ranges"
		ASN=$(curl -s "https://api.hackertarget.com/aslookup/?q=$domain" | head -1 | cut -d' ' -f1)
		if [[ ! -z "$ASN" && "$ASN" != "error" ]]; then
			echo "ASN: $ASN" | anew -q osint/asn_info.txt
			curl -s "https://api.hackertarget.com/aslookup/?q=$ASN" | tail -n +2 | sort -u > osint/ip_ranges.txt
			notification "ASN $ASN and IP ranges discovered" good
		fi
		end_func "Results are saved in osint/asn_info.txt and osint/ip_ranges.txt" "${FUNCNAME[0]}"
	fi
}
...
function mobile_recon() {
	mkdir -p osint
	if { [[ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ]] || [[ $DIFF == true ]]; } && [[ $OSINT == true ]]; then
		start_func "${FUNCNAME[0]}" "Scanning for Mobile API endpoints"
		if [[ -f "webs/url_extract.txt" ]]; then
			grep -Ei "api|v1|v2|v3|mobile|firebase|amplify|s3|bucket" webs/url_extract.txt | sort -u > osint/mobile_endpoints.txt
			notification "Mobile endpoints extraction finished" good
		fi
		end_func "Results are saved in osint/mobile_endpoints.txt" "${FUNCNAME[0]}"
	fi
}
...
function subdomains_full() {
	...
	# Process subdomains
	if [[ -s "subdomains/subdomains.txt" ]]; then
		# BUG FIX #3: Filtering CDN Noise and False Positives in Passive Mode
		CDN_LIST="cloudfront|akamai|googleapis|aws|azure|blogspot|wordpress|github|wp-content|wp-includes|cdn"
		cat "subdomains/subdomains.txt" | grep -vE "\.($CDN_LIST)\." | sort -u > "subdomains/subdomains_clean.txt"
		mv "subdomains/subdomains_clean.txt" "subdomains/subdomains.txt"
...
function iran_mode_init() {
	if [[ $IRAN_MODE == true ]]; then
		notification "Iran Mode Enabled: Optimizing for restricted environments" info
		GOOGLE_DORKS=false
		GITHUB_DORKS=false
		# Use local resolvers if available or DoH
		if [[ -n $DOH_RESOLVER ]]; then
			notification "Using DoH Resolver: $DOH_RESOLVER" info
		fi
		# Fallback for GitHub
		if [[ -n $GH_PROXY ]]; then
			notification "GitHub Proxy enabled: $GH_PROXY" info
		fi
	fi
}

function proxy_wrapper() {
	if [[ $PROXY_CHAIN_ENABLED == true && -f $PROXIES_FILE ]]; then
		# Simple proxy wrapper could be just exporting http_proxy or using proxychains
		# For now, let's assume we use it for specific tools that support it
		echo " [PROXY] Running with proxy support"
	fi
}

function low_ram_optimization() {
	if [[ $LOW_RAM_MODE == true ]]; then
		notification "Low RAM Mode: Reducing threads to save memory" warn
		FFUF_THREADS=10
		HTTPX_THREADS=20
		RESOLVE_DOMAINS_THREADS=50
	fi
}

function tld_aware_wordlist() {
	if [[ $TLD_AWARE_WORDLISTS == true ]]; then
		local tld=$(echo "$domain" | rev | cut -d. -f1 | rev)
		if [[ -f "${tools}/subdomains_${tld}.txt" ]]; then
			notification "TLD-aware Wordlist found: subdomains_${tld}.txt" good
			subs_wordlist="${tools}/subdomains_${tld}.txt"
		fi
	fi
}
...
function start() {
	...
	# BUG FIX #2: Check for missing API keys (Silent Failure)
	REQUIRED_KEYS=("SHODAN_API_KEY" "WHOISXML_API" "GITHUB_TOKENS")
	for key in "${REQUIRED_KEYS[@]}"; do
		if [[ -z ${!key} ]]; then
			printf "%b[%s] WARNING: %s is not set. Some results will be incomplete.%b\n" "$yellow" "$(date +'%Y-%m-%d %H:%M:%S')" "$key" "$reset"
		fi
	done
